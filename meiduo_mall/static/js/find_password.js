var vm = new Vue({
    ...
    methods: {
        ...
        generate_image_code: function(){
            // 生成一个编号
            // 严格一点的使用uuid保证编号"image_code_%s" % image_code_id一， 不是很严谨的情况下，也可以使用时间戳
        ...
        form_1_on_submit: function(){
            ...
            if (this.error_username == false && this.error_image_code == false) {
                axios.get(this.host+'/accounts/' + this.username + '/sms/token/?text='+ this.image_code +
        ...
        send_sms_code: function(){
            ...
            axios.get(this.host+'/sms_codes/?mobile='+ this.mobile, {
                    responseType: 'json'
                })
                .then(response => {
                    // 表示后端发送短信成功
        ...
        form_2_on_submit: function(){
            ...
            if (this.error_sms_code == false) {
                axios.get(this.host + '/accounts/' + this.username + '/password/token/?sms_code=' + this.sms_code, {
                    ...
                    .then(response => {
                        this.user_id = response.data.user_id;  //此处为需要传参的变量
                ...
                axios.post(this.host + '/users/'+ this.user_id +'/password/', {
                    ...
                    .then(response => {
                        if (response.status == '400') {
                            this.error_sms_code_message = '填写数据不完整';
                            this.error_sms_code = true;
                        } else if(response.status == '404'){
                            this.error_sms_code_message = '两个密码不一致';
                            this.error_sms_code = true;
                        } else if (response.status == '401'){
                            this.error_sms_code_message = '用户不存在';
                            this.error_sms_code = true;
                        }else {
